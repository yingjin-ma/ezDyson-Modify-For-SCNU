...
double Gauss::GaussValue(double x, double y, double z, double x0, double y0, double z0) const
{
  ...
  if(2==dx2y2)
	...
	switch (fa)
	  {
	  case 1:
            rf = 0.5*(2*Yat2*Yat-3*Xat2*Yat-3*Zat2*Yat); //2*pow(Yat,3)-3*pow(Xat,2)*Yat-3*pow(Zat,2)*Yat;
	    break;
	  case 3:
            rf = sqrt(15)/2*(Yat*(Xat2-Zat2)); //Yat*(pow(Zat,2)-pow(Xat,2));
	    break;
	  case 4:
            rf = 0.5*(2*Zat2*Zat-3*Xat2*Zat-3*Yat2*Zat); //2*pow(Zat,3)-3*pow(Xat,2)*Zat-3*pow(Yat,2)*Zat;
	    break;
          case 5:
            rf = sqrt(15)/2*(Xat*(Zat2-Yat2)); //Xat*(pow(Zat,2)-pow(Yat,2));
            break;
	  case 6:
            rf = sqrt(15)/2*(Zat*(Xat2-Yat2)); //Zat*(pow(Xat,2)-pow(Yat,2));
	    break;
	  case 7:
            rf = 0.5*(2*Xat2*Xat-3*Yat2*Xat-3*Zat2*Xat); //2*pow(Xat,3)-3*pow(Yat,2)*Xat-3*pow(Zat,2)*Xat;
	    break;
	  }    
	for (int i=0; i<ncontr; i++)
